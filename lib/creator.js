...
// Edits fully replace the page set rather than reconciling a partial
// reorder/merge of old and new pages — simpler and safer, at the cost of
// re-uploading everything even for a one-page fix.
export async function updateMangaChapter(actorId, isAdmin, seriesId, chapterId, { chapterNumber, title, pageBuffers }) {
  const { contentType } = await assertCanEditChapter(seriesId, chapterId, actorId, isAdmin);
  ...
  const { rows: oldPages } = await query('SELECT storage_path FROM chapter_pages WHERE chapter_id = $1', [chapterId]);
  await deleteObjects(oldPages.map((p) => p.storage_path));
  ...
  for (const buffer of pageBuffers) {
    ...
    const storagePath = `series/${seriesId}/chapters/${chapterId}/${Date.now()}-${pageNumber}.webp`;
    const { cdnUrl } = await uploadPage(buffer, storagePath);
    await query(
      `INSERT INTO chapter_pages (chapter_id, page_number, cdn_image_url, storage_path)
       VALUES ($1, $2, $3, $4)`,
