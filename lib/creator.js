...
export async function addSeriesReviewMessage(actorId, isAdmin, seriesId, { body, imageBuffer, kind = 'message' } = {}) {
  const creatorId = await assertCanManageSeries(seriesId, actorId, isAdmin);
  ...
  // Only an admin posting notifies — the creator already has /dashboard/threads,
  // and admins already have /admin/threads for their own side of the conversation.
  if (isAdmin) {
...
export async function addMangaChapter(creatorId, seriesId, { chapterNumber, title, pageBuffers }) {
  ...
  await notifications.notifyFollowersNewChapter(seriesId, chapterId, chapterNumber, creatorId);
...
export async function addNovelChapter(creatorId, seriesId, { chapterNumber, title, body }) {
  ...
  await notifications.notifyFollowersNewChapter(seriesId, rows[0].id, chapterNumber, creatorId);
...
export async function addChapterReviewMessage(actorId, isAdmin, seriesId, chapterId, { body, imageBuffer, kind = 'message' } = {}) {
  const { creatorId } = await assertCanEditChapter(seriesId, chapterId, actorId, isAdmin);
  const cleanBody = body?.trim() || null;
  if (kind === 'message' && !cleanBody && !imageBuffer) {
    throw new Error('Message must include text or an image');
  }
...
export async function getMySeries(creatorId) {
  const { rows } = await query(
    `SELECT s.*, (SELECT COUNT(*) FROM chapters c WHERE c.series_id = s.id) AS chapter_count,
            (SELECT COUNT(*) FROM series_follows sf WHERE sf.series_id = s.id) AS follower_count
...
export async function markThreadRead(userId, threadType, threadId) {
  ...
  // Reading a thread (opening its page, hitting "mark as read", or posting
  // into it yourself) should also clear any notification-tab entry telling
  // you about a message you've now plainly already seen — otherwise the two
  // read states drift apart and the notification stays stuck unread.
  await notifications.markThreadNotificationsRead(userId, threadType, threadId);
}
