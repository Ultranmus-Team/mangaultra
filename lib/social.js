...
import { maskAdminIdentity } from './util.js';
...
export async function addChapterComment(authorId, seriesId, chapterId, { body, imageBuffer, parentId } = {}) {
  ...
  // Every notification about this comment points at the thread root (the
  // top-level comment), not the newly-inserted row itself — that's what the
  // dedicated /comment/[commentId] thread page (see getChapterComment below)
  // is keyed off, whether this post is that root comment or a reply under it.
  const threadRootId = parentTopLevelId || rows[0].id;

  // A reply notifies the top-level comment's author, not the direct parent
  // (a reply to a reply is re-parented above, so its own "parent" is always
  // the top-level comment).
  let parentAuthorId = null;
  if (parentTopLevelId) {
    const { rows: parentRows } = await query('SELECT author_id FROM chapter_comments WHERE id = $1', [
      parentTopLevelId,
    ]);
    parentAuthorId = parentRows[0]?.author_id || null;
    if (parentAuthorId) {
  ...
  // The chapter's own creator also hears about any comment on their work —
  // skipped when they're the one commenting, or when they already got the
  // comment_reply notification above for this exact same post (they're the
  // parent's author), so they're never notified twice for one event.
  const { rows: seriesRows } = await query('SELECT creator_id FROM series WHERE id = $1', [seriesId]);
  const creatorId = seriesRows[0]?.creator_id;
  if (creatorId && creatorId !== parentAuthorId) {
    await notifications.createNotification({
      userId: creatorId,
      type: 'new_comment',
      actorId: authorId,
...
// One top-level comment (for the dedicated thread page a comment_reply /
// new_comment notification links to). A reply's id redirects to its own
// parent instead of 404ing — notifications always store the thread root, but
// this stays defensive against any other caller passing a reply's id.
export async function getChapterComment(chapterId, commentId) {
  const { rows } = await query(
    `SELECT c.id, c.body, c.image_url, c.created_at, c.author_id, c.parent_id,
            p.username AS author_username, p.avatar_url AS author_avatar_url, p.role
     FROM chapter_comments c
     JOIN profiles p ON p.id = c.author_id
     WHERE c.id = $1 AND c.chapter_id = $2`,
    [commentId, chapterId]
  );
  if (rows.length === 0) return null;
  return maskAdminIdentity(rows[0], { usernameKey: 'author_username', avatarKey: 'author_avatar_url' });
}
...
export async function getChapterComments(chapterId, { limit = COMMENTS_PAGE_SIZE, offset = 0 } = {}) {
  ...
  const comments = rows.map((r) => ({
    ...maskAdminIdentity(r, { usernameKey: 'author_username', avatarKey: 'author_avatar_url' }),
...
export async function getChapterCommentReplies(chapterId, parentId, { limit = REPLIES_PAGE_SIZE, offset = 0 } = {}) {
  ...
  const replies = rows.map((r) => maskAdminIdentity(r, { usernameKey: 'author_username', avatarKey: 'author_avatar_url' }));
  const { rows: countRows } = await query('SELECT COUNT(*) FROM chapter_comments WHERE parent_id = $1', [parentId]);
  const total = Number(countRows[0].count);
  return { replies, total, hasMore: offset + rows.length < total };
}
...
export async function setChapterReaction(userId, chapterId, reactionType) {
  ...
  if (reactionType === null) {
    ...
    if (existing.length > 0 && existing[0].reaction_type === reactionType) {
      ...
      if (chapterRows[0]) {
        const { rows: seriesRows } = await query('SELECT creator_id FROM series WHERE id = $1', [
          chapterRows[0].series_id,
        ]);
        if (seriesRows[0]) {
          await notifications.createNotification({
            ...
            type: 'reaction',
            actorId: userId,
            seriesId: chapterRows[0].series_id,
            chapterId,
            body: reactionType,
          });
        }
